    ...
    public JmmParserResult parse(String jmmCode, String startingRule, Map<String, String> config) {
        ...
        try {
            ...
            return AntlrParser.parse(lex, parser, startingRule)
                    ...
                    .orElseGet(() -> JmmParserResult.newError(new Report(ReportType.ERROR, Stage.SYNTATIC, -1, "There were " + Integer.toString(parser.getNumberOfSyntaxErrors()) + " syntax errors during parsing, terminating")));

        } catch (Exception e) {
            // There ERRORwas an uncaught exception during parsing, create an error JmmParserResult without root node
